...
void serial_init()
{
   //UBRR = 1666; //used to set the baud rate USART Buad Rate Register
   UCSRB = (1 << TXEN) | (1 << RXEN);                  // Enable transmitter and receiver
   UCSRC = (1 << UCSZ1) | (1 << UCSZ0) | (1 << URSEL); // Enable UCSRC and set data frame to 8-bits
   UBRRL = 0x05;                                       // Set the Baud Rate Register Low value
}
